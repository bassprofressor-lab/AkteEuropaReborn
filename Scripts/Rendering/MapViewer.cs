...
public partial class MapViewer : Node2D
{
    ...
    public override void _Ready()
    {
        ...
        if (_mineCheck)
        {
            GD.Print(_entities.MineCheck());
            GetTree().Quit(0);
            return;
        }
        if (_entladenCheck)
        {
            GD.Print(_entities.EntladenCheck());
            GetTree().Quit(0);
            return;
        }
        if (_beladenCheck)
        {
            GD.Print(_entities.BeladenCheck());
            GetTree().Quit(0);
            return;
        }
        if (_brandCheck)
        {
            GD.Print(_entities.BrandCheck());
            GetTree().Quit(0);
            return;
        }
        if (_bandCheck)
        {
            GD.Print(_entities.BandCheck());
            GetTree().Quit(0);
            return;
        }
        if (_sektorCheck)
        ...
        if (_rampenCheck)
        {
            GD.Print(_entities.RampenCheck());
            GetTree().Quit(0);
            return;
        }
        if (_transportCheck)
    ...
    private void ParseCmdline()
    {
        ...
        foreach (string a in Core.CommandLine.Args)
        {
            ...
            {
                var t = a["--boden-um=".Length..].Split(',');
                if (t.Length >= 2)
                { MapEntityLayer.BodenUmCol = t[0].ToInt(); MapEntityLayer.BodenUmRow = t[1].ToInt(); }
                if (t.Length >= 3) MapEntityLayer.BodenUmWeite = t[2].ToInt();
            }
